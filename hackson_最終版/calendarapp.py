"""
ターミナル上でコード上にお気に入りの食堂の名称を入力するとターミナル上にお気に入りの食堂が開催している日付と場所などの簡易的な情報が閲覧できる
...
#カレンダーの年と月をここで入力
while True:
    ym_input = input("カレンダーを表示したい年月を入力してください (例: 2025-09): ")
    try:
        year_str, month_str = ym_input.split("-")
        year = int(year_str)
        month = int(month_str)
        if 1 <= month <= 12:
            break
        ...
        print("入力形式が正しくありません。YYYY-MM の形で入力してください。")
